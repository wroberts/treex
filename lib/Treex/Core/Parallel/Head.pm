...
sub _run_job_script {
    ...
    if ( $self->local ) {
        ...
        $qsub_opts .= ' -p ' . $self->priority; # not UPV/EHU
        ...
        #open my $QSUB, "cd $workdir && TREEX_PARALLEL=1 qsub $qsub_opts $script_filename |" or log_fatal $!;    ## no critic (ProhibitTwoArgOpen) # UPV/EHU
...
sub _execute_scenario {
    ...
    if ($workdir =~ /\{(N+)\}/) {
        ...
        do {
            ...
            @existing_dirs = grep {-e $_} bsd_glob "$dir_wildcard";    # separate var because of troubles with glob context
            }
            while (@existing_dirs);
    }
    if ($workdir =~ /\{(X+)\}/) {
